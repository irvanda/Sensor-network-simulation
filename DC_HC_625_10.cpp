...
//  Packet Forwarding in WSN - Broadcast Flooding with Duty Cycle
...
struct UE { // UE = User Entity / node
	...
	int dc_count; // counter for Duty Cycle
...
int main(){
	...
	// ofstream check;
	// check.open("check_DC.txt");
	ofstream trace;
	trace.open("trace_DC_HC_625_10.txt");
	ofstream genPkt;
	genPkt.open("genPkt_DC_HC_625_10.txt");
	...
	int gen_prob = 10; // 5% packet generation
	int gen_gap=100000;
	...
	int dc_slot = 100000;
	double dc = 0.5;
	int active_slot = 50000; // dc_slot * dc
	...
	for (i=0; i<n_n; i++)
	{
		node[i].dc_count = rand()%active_slot;
		node[i].active = true;
	...
	for (slot =0; slot <= simSlot; slot++)
	{
		...
		for (i=0; i<n_n; i++)
		{
			if (node[i].dc_count >= active_slot)
				if (node[i].active)
				{
					node[i].active = false;
					node[i].dc_count = 0;
		...
		for (i=0; i<n_n; i++)
		{
			...
			//increase dc counter
			node[i].dc_count = node[i].dc_count + 1;
		}
