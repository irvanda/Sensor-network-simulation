...
int main(){
	...
	// check.open("check_HC.txt");
		ofstream trace;
	trace.open("trace_HC_1600_5.txt");
	ofstream genPkt;
	genPkt.open("genPkt_HC_1600_5.txt");
			...
			if(transmit)
			{
				...
				for (i=0; i<n_n; i++)
				{
					...
					else if(size == 1) // push packet to buffer
					{
						...
						if (i == sink)
						{
							trace << "r; " << slot << "; " << i << "; " << node[i].r_buff[0].s_node << "; " << node[i].r_buff[0].p_id << "; "<< node[i].r_buff[0].nslot << "; " << node[i].r_buff[0].nhop << endl;
						}
